...
export class MetadataService {
  ...
  public async createMetadata(
    ...
    const offerIds: string[] = (await this.offerPrismaService.getOffersByOrderId(serviceProcess.orderId, OFFER_STATES_TO_SHOW)).map((offer: Offer) => {
      return offer.id;
    });
    const machineAssignment: MachineAssignment[] = await this.serviceProcessPrismaService.getMachineAssignment(serviceProcessId);
    const invoiceIds: string[] = (await this.invoicePrismaService.getInvoiceByServiceProcessId(serviceProcessId)).map((invoice: Invoice) => {
      return invoice.id;
    });
