import {
  AmqpBrokerQueues, CompanyAmqpDto, CompanyAmqpMock, CompanyMessagePatterns,
...
describe('InvoicesController', () => {
  ...
  it('should find all invoices by OrderId', async () => {
    ...
    const res: InvoiceDto[] = await controller.findAll('Order');
  ...
  it('should find an invoice by its id', async () => {
    ...
    const res: InvoiceDto = await controller.findOne(InvoiceMocks[0].id);
    expect(sendRequestSpy).toHaveBeenCalledWith(InvoiceMessagePatterns.READ_BY_ID, InvoiceMocks[0].id);
    expect(res).toEqual(expectedReturnValue);
  });
