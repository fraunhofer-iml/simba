...
import { ServiceStatesEnum, UserRoles } from '@ap3/util';
import { Roles } from 'nest-keycloak-connect';
import { Body, Controller, Delete, Get, HttpException, HttpStatus, Param, Post, Query, Request } from '@nestjs/common';
...
export class OrdersController {
  ...
  async create(@Request() req, @Body() createOrderDto: CreateOrderDto): Promise<OrderOverviewDto> {
    if (req.user.company === createOrderDto.customerId) {
  ...
  async findAll(
    ...
    if (!req.user.realm_access.roles.includes(UserRoles.ADMIN) || !companyId) {
