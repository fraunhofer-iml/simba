...
export class OrdersOverviewComponent implements OnInit {
  ...
  private loadOrders(): void {
    this.orderService.getOrders().subscribe((orders: OrderOverviewDto[]) => {
      this.dataSourceOpen.data = orders.filter((order: OrderOverviewDto) => order.status !== OrderStatus.produced);
      this.dataSourceClosed.data = orders.filter((order: OrderOverviewDto) => order.status === OrderStatus.produced);
    });
  }
